...
export class DataQueue extends Base {
    ...
    private renderDataQueuePanel(): string {
        ...
        ${Components.divider()}
        ${Components.button("Send data", "sendData", { action: ACTION_SEND, icon: { name: "arrow-right", left: true } })}
        ${Components.button("Clear", "clear", { action: ACTION_CLEAR, appearance: "secondary", icon: { name: "clear-all", left: true } })}
        `;
    }
}

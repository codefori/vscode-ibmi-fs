...
export namespace WrksplfActions {
   ...
   * is only visible while `activeWebviewPanelId == 'wrksplfView'`, so it always matches what the
   * user is looking at.
   */
  ...
  export const register = (context: vscode.ExtensionContext) => {
    ...
    context.subscriptions.push(
      vscode.commands.registerCommand('vscode-ibmi-fs.refreshWrksplf', async (isAutoRefresh: boolean = false) => {
        const refresh = currentPanel && panelRefreshers.get(currentPanel);
        if (refresh) {
          await refresh(isAutoRefresh);
          return;
        }
        vscode.window.showWarningMessage(vscode.l10n.t("No spooled files view found to refresh"));
      })
    );
